...
- 默认管理员登录，并支持在后台新增、修改普通用户；管理员可以查看所有用户的任务和最近图片记录。
...
```bash
APP_USERNAME=管理员账号 APP_PASSWORD=管理员密码 npm start
```

管理员登录后可以在“接口配置”里填写 API Key、接口地址、模型和默认比例；保存后新任务会立即使用，无需重启容器。管理员也可以在“用户管理”里新增或修改用户，普通用户只使用后台统一配置好的图片接口。
...
- 环境变量：只需要设置 `APP_USERNAME`、`APP_PASSWORD`
- 图片接口：登录管理员后台后，在“接口配置”里填写 API Key、接口地址、模型和默认比例；这些配置保存在 `/data/.personal-data.json`
